...
app.controller('ProfileController', ["$scope", "$http", "profileService", "referralService", function ($scope, $http, profileService, referralService) {
	...
	$scope.addProfile = function (profile) {
		//Combine next step date and time into one field
		profile.nextSteps = profile.nextStepDate + " " + profile.nextStepTime;

		//Update referral to show new next step date, if not empty
		profile.referral.nextStepDate = profile.nextSteps;

		referralService.put(profile.referral, function () {
